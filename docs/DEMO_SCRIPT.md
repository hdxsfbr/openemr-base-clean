...
| 0:30 | AF-DQ-A2 dashboard; click the **Clinical Copilot** menu item and then "What changed since the last visit?"; the right drawer opens and its progress line animates | "Physician, 90 seconds before a visit. Nothing is retrieved until I ask. It retrieves six sections through OpenEMR's own access checks, the model writes claims, and a deterministic verifier checks every one against the records before anything renders." | normal sourced workflow |
