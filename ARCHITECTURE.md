...
| Lab PDF and intake-form ingestion; round-tripping derived records without duplicates | Slice 1A/1B provides the synthetic lab-PDF upload and deterministic, review-only cited preview (`docs/week2/slice1a-upload-contract.md`, `docs/week2/slice1b-extraction-preview.md`). Slice 2A/2B uses that same patient-bound source seam for the synthetic intake PDF and its deterministic, partial-aware cited preview (`docs/week2/slice2a-intake-contract.md`, `docs/week2/slice2b-intake-extraction-preview.md`). Slice 2C adds a shared automatic lab/intake gate and type-bounded preview telemetry (`docs/week2/slice2c-release-gate.md`). Authenticated browser/deployment proof, review/promotion, overlay, and round-trip records remain Week 2 work under ADR-0008, ADR-0009, ADR-0011, and ADR-0012. |
| Guideline evidence through hybrid RAG | Slice 3A/#40 implements the offline approved USPSTF corpus manifest, deterministic heading-aware chunks, identical-set FTS5/FAISS artifacts, active pointer, pinned BGE model/tokenizer/runtime hashes, and strict `EvidenceQuery`/guideline source-citation-claim exports. Slice 3B/#41 adds the local FTS5/BGE/RRF/MiniLM engine with hash-bound setup-time model provisioning, exact excerpts, and fail-closed retrieval (`docs/week2/slice3b-guideline-retrieval.md`). Slice 3C/#42 adds the strict local worker envelope, atomic terminal limitation behavior, cancellation/stale/duplicate handoff controls, and PHI-free correlated worker telemetry. Slice 3D/#43 adds an explicit finite request through the existing chart ticket, deterministic active-corpus resolver/verifier, fresh-ticket source reauthorization, and a separate no-applicability UI lane (`docs/week2/slice3d-guideline-release.md`). It does not add supervisor routing or automatic retrieval on chart questions. |
