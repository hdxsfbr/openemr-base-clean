...
pushed=0
for name in anthropic_api_key langfuse_public_key langfuse_secret_key; do
...
if [[ "${pushed}" -gt 0 ]]; then
    ...
    curl -s --max-time 20 "https://$(ssh "${ssh_options[@]}" "${ssh_target}" 'grep PUBLIC_HOSTNAME /opt/agentforge/.env | cut -d= -f2')/copilot-api/ready" \
        | python3 -c 'import sys,json; d=json.load(sys.stdin); print(d["status"]); [print(f"  {x[\"name\"]}: {x[\"detail\"]}") for x in d["dependencies"]]' || true
fi
