...
| Guideline retrieval latency | p50 at or below 500 ms; p95 at or below 2.0 s under a 2 s hard deadline | Owner risk acceptance 2026-09-22, recorded in ADR-0014's amendment: the 1.5 s target was not met on the deployed synthetic environment (five completed finite-topic requests: worker p50 1372.1 ms, p95 1725.3 ms; endpoint p95 1753.1 ms). The p50 target was also missed. A prior first UI request instead reached the fail-closed typed `guideline_retrieval_timeout` limitation and displayed no claim; it is excluded from the completed-only sample and remains unfixed in [#44](https://labs.gauntletai.com/andrebatista/andrebatista-openemr-base-clean/-/work_items/44). Under the operational budget table's hard-gate column, the unchanged 2 s deadline is release-blocking; the p50 is an unmet performance target requiring follow-up, not a silent pass. The per-request deadline, sparse+dense fusion, pinned reranker, corpus/model revisions, and no-fallback rule are unchanged. This small n=5 sample is evidence of the accepted p95 gate only, not a capacity or mixed-load result. Slice 3B's 24-case local benchmark remains 267.11 ms p95 (sparse 0.53 ms, dense 10.26 ms, fusion 0.03 ms, rerank 259.92 ms) on a 32-logical-CPU host and is not co-located deployment proof (`docs/research/week2-retrieval-benchmark/engine-results-2026-09-22.json`). |
