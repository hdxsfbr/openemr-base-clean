...
| Guideline retrieval latency | p95 at or below 1.5 s under a 2 s hard deadline | Evidence must still fit the pre-visit workflow. Report keyword, dense, fusion, rerank, and total stages; returning fewer candidates or skipping rerank cannot make a run green. Slice 3B local evidence (24 synthetic finite-concept cases, 2026-09-22) measured total p95 267.11 ms: sparse 0.53 ms, dense 10.26 ms, fusion 0.03 ms, rerank 259.92 ms; the 32-logical-CPU host result is not the later co-located deployment gate (`docs/research/week2-retrieval-benchmark/engine-results-2026-09-22.json`). |
